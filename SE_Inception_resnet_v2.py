...
class SE_Inception_resnet_v2():
    ...
    def Build_SEnet(self, input_x):
        input_x = tf.pad(input_x, [[0, 0], [16, 16], [16, 16], [0, 0]])
        # size 32 -> 96
...
# image_size = 64, img_channels = 3, class_num = 320
...
with tf.Session() as sess:
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        train_dic = {}
        with open('./DatasetA_train_20180813/train.txt') as fp:
            for line in fp:
                wn, name = line.split('\t')
        ...
        for step in range(1, iteration + 1):
            if pre_index + batch_size < 38221:
                input_x = img_name[pre_index: pre_index + batch_size]
                input_y = img_label[pre_index: pre_index + batch_size]
            else:
                input_x = img_name[pre_index:]
                input_y = img_label[pre_index:]
            ...
            batch_loss = sess.run(cost, feed_dict=train_feed_dict)
            batch_acc = accuracy.eval(feed_dict=train_feed_dict)

            train_loss += batch_loss
            train_acc += batch_acc
            pre_index += batch_size
            print("batch:%d, batch_loss:%4f, batch_acc:%4f\n" % (step, batch_loss, batch_acc))
